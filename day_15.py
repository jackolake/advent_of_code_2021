...
if __name__ == '__main__':
    ...
    # Explore
    while explorer_heap:
        explorer_risk, explorer_node = heapq.heappop(explorer_heap)  # least cost
        if explorer_node not in visited_nodes:
            visited_nodes.add(explorer_node)
            ...
            for mod_row, mod_col in [(1, 0), (0, 1), (-1, 0), (0, -1)]:  # Down, Right, Up, Left
                ...
                if 0 <= test_coord[0] < risk.shape[0] and 0 <= test_coord[1] < risk.shape[1]:
                    if explorer_risk + risk.iloc[test_coord] < risk_from_start[test_coord]:
                        risk_from_start[test_coord] = explorer_risk + risk.iloc[test_coord]
                    if test_coord not in visited_nodes:
